...
````SQL
--******************************************************************************
--PREPARATION ENVIRONNEMENT DE BASE
--******************************************************************************

create sequence SEQ_PK_images_DJ;

CREATE TABLE images_DJ
  (numero NUMBER(38), image_blob blob, information VARCHAR2(250));

CREATE OR REPLACE TRIGGER TR_images_DJ_PK
BEFORE INSERT ON images_DJ
FOR EACH ROW
BEGIN
  SELECT SEQ_PK_images_DJ.Nextval INTO :New.numero FROM DUAL;
END;
/
-- DROP SEQUENCE SEQ_PK_images_DJ;
-- DROP TRIGGER TR_images_DJ_PK;
-- DROP TABLE images_DJ;

--******************************************************************************
--CREATION DU PACKAGE NECESSAIRE POUR LES TESTS
--******************************************************************************
...
DECLARE
  res_img_blob blob;
BEGIN
   res_img_blob:=pkg_ordimage_DJ.GitHubProfil;
   insert into images_DJ(image_blob,information) values(res_img_blob,'pkg_ordimage_DJ.GitHubProfil - Image créée avec des formes aléatoires et un effet miroir horizontal dans le style des photos de profil GitHub');
END;
/
...
DECLARE
 	img_blob blob;
BEGIN
  SELECT image_blob INTO img_blob FROM images_DJ WHERE numero = 1;
    pkg_ordimage_DJ.displayAttributs(img_blob);
END;
/

-- Resultat

--  ATTRIBUTS of the image BLOB file
--  -------
--  File format is : PNGF
--  Height is : 420 px
--  Width is : 420 px
--  Compression format is : DEFLATE
--  Content format is : 24BITRGB
--  Content length is : 3966 Bytes
...
BEGIN
 SELECT image_blob INTO img_blob FROM images_DJ WHERE numero = 1;
 res_img_blob:=pkg_ordimage_DJ.cutBorderTop(img_blob,30);
 insert into images_DJ(image_blob, information) values(res_img_blob, 'pkg_ordimage_DJ.cutBorderTop - Image à laquelle on a coupé sa bordure du haut de 30 pixels');
END;
/
...
BEGIN
 SELECT image_blob INTO img_blob FROM images_DJ WHERE numero = 1;
 res_img_blob:=pkg_ordimage_DJ.cutBorderBottom(img_blob, 30);
 insert into images_DJ(image_blob, information) values(res_img_blob, 'pkg_ordimage_DJ.cutBorderBottom - Image à laquelle on a coupé sa bordure du bas de 30 pixels');
END;
/
...
BEGIN
 SELECT image_blob INTO img_blob FROM images_DJ WHERE numero = 1;
 res_img_blob:=pkg_ordimage_DJ.cutBorderLeft(img_blob, 30);
 insert into images_DJ(image_blob, information) values(res_img_blob, 'pkg_ordimage_DJ.cutBorderLeft - Image à laquelle on a coupé sa bordure de gauche de 30 pixels');
END;
/
...
BEGIN
 SELECT image_blob INTO img_blob FROM images_DJ WHERE numero = 1;
 res_img_blob:=pkg_ordimage_DJ.cutBorderRight(img_blob, 30);
 insert into images_DJ(image_blob, information) values(res_img_blob, 'pkg_ordimage_DJ.cutBorderRight - Image à laquelle on a coupé sa bordure de droite de 30 pixels');
END;
/
...
BEGIN
 SELECT image_blob INTO img_blob FROM images_DJ WHERE numero = 1;
 res_img_blob:=pkg_ordimage_DJ.cutBorder(img_blob, 30,30,30,30);
 insert into images_DJ(image_blob, information) values(res_img_blob, 'pkg_ordimage_DJ.cutBorder - Image à laquelle on a coupé 30 pixels de chaque côté');
END;
/
...
DECLARE
  ...
  res_img_blob blob;
BEGIN
  SELECT image_blob INTO img_blob1 FROM images_DJ WHERE numero = 6;
  SELECT image_blob INTO img_blob2 FROM images_DJ WHERE numero = 6;
  res_img_blob:=pkg_ordimage_DJ.pastSide(img_blob1,img_blob2);
  insert into images_DJ(image_blob, information) values(res_img_blob, 'pkg_ordimage_DJ.pastSide - Composition de deux images collées l une à côté de l autre');
END;
/
...
DECLARE
  ...
  res_img_blob blob;
BEGIN
  SELECT image_blob INTO img_blob1 FROM images_DJ WHERE numero = 7;
  SELECT image_blob INTO img_blob2 FROM images_DJ WHERE numero = 7;
  res_img_blob:=pkg_ordimage_DJ.pastUp(img_blob1,img_blob2);
  insert into images_DJ(image_blob, information) values(res_img_blob, 'pkg_ordimage_DJ.pastUp - Composition deux images collées l une au dessus de l autre');
END;
/
...
DECLARE
  ...
  res_img_blob blob;
BEGIN
 SELECT image_blob INTO img_blob FROM images_DJ WHERE numero = 8;
 res_img_blob:=pkg_ordimage_DJ.addBorderTop(img_blob,30);
 insert into images_DJ(image_blob, information) values(res_img_blob, 'pkg_ordimage_DJ.cutBorderTop - Image à laquelle on a ajouté une bordure du haut de 30 pixels');
END;
/
...
BEGIN
 SELECT image_blob INTO img_blob FROM images_DJ WHERE numero = 8;
 res_img_blob:=pkg_ordimage_DJ.addBorderBottom(img_blob, 30);
 insert into images_DJ(image_blob, information) values(res_img_blob, 'pkg_ordimage_DJ.cutBorderBottom - Image à laquelle on a ajouté une bordure du bas de 30 pixels');
END;
/
...
BEGIN
 SELECT image_blob INTO img_blob FROM images_DJ WHERE numero = 8;
 res_img_blob:=pkg_ordimage_DJ.addBorderLeft(img_blob, 30);
 insert into images_DJ(image_blob, information) values(res_img_blob, 'pkg_ordimage_DJ.cutBorderLeft - Image à laquelle on a ajouté une bordure de gauche de 30 pixels');
END;
/
...
BEGIN
 SELECT image_blob INTO img_blob FROM images_DJ WHERE numero = 8;
 res_img_blob:=pkg_ordimage_DJ.addBorderRight(img_blob, 30);
 insert into images_DJ(image_blob, information) values(res_img_blob, 'pkg_ordimage_DJ.cutBorderRight - Image à laquelle on a ajouté une bordure de droite de 30 pixels');
END;
/
...
BEGIN
  SELECT image_blob INTO img_blob FROM images_DJ WHERE numero = 8;
  res_img_blob:=pkg_ordimage_DJ.addBorder(img_blob, 30,30,30,30);
  insert into images_DJ(image_blob, information) values(res_img_blob, 'pkg_ordimage_DJ.cutBorder - Image à laquelle on a ajouté de chaque côté des bordures de 30 pixels');
END;
/
...
BEGIN
  SELECT image_blob INTO img_blob FROM images_DJ WHERE numero = 1;
  res_img_blob:=pkg_ordimage_DJ.polaroidFrame(img_blob);
  insert into images_DJ(image_blob, information) values(res_img_blob, 'pkg_ordimage_DJ.polaroidFrame - Image avec un cadre façon "Polaroid"');
END;
/
...
BEGIN
  SELECT image_blob INTO img_blob FROM images_DJ WHERE numero = 1;
  res_img_blob:=pkg_ordimage_DJ.polaroidFrame(img_blob, 'cool');
  insert into images_DJ(image_blob, information) values(res_img_blob, 'pkg_ordimage_DJ.polaroidFrame - Image avec un cadre façon "Polaroid" et un text');
END;
/
...
BEGIN
  SELECT image_blob INTO img_blob FROM images_DJ WHERE numero = 8;
  res_img_blob:=pkg_ordimage_DJ.facebookProfilCut(img_blob);
  insert into images_DJ(image_blob, information) values(res_img_blob, 'pkg_ordimage_DJ.facebookProfilCut - Image photo de profil facebook découpée depuis une maquette');
END;
/
...
BEGIN
  SELECT image_blob INTO img_blob FROM images_DJ WHERE numero = 8;
  res_img_blob:=pkg_ordimage_DJ.facebookBackgroundCut(img_blob);
  insert into images_DJ(image_blob, information) values(res_img_blob, 'pkg_ordimage_DJ.facebookBackgroundCut - Image photo de background facebook découpée depuis une maquette');
END;
/
